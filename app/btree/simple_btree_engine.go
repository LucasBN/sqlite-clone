...
import (
	...
	"encoding/binary"
	"errors"
	"github/com/lucasbn/sqlite-clone/app/pager"

	"github.com/samber/lo"
)

type SimpleBTreeEngine struct {
	Pager   pager.Pager
	Cursors map[uint64]*cursor
}

var _ BTreeEngine = &SimpleBTreeEngine{}

func NewSimpleBTreeEngine(pager pager.Pager) (*SimpleBTreeEngine, error) {
	return &SimpleBTreeEngine{
		Pager:   pager,
...
func (b *SimpleBTreeEngine) NewCursor(id uint64, rootPageNum uint64) (bool, error) {
...
// index.
func (b *SimpleBTreeEngine) RewindCursor(id uint64) (bool, error) {
...
// index.
func (b *SimpleBTreeEngine) AdvanceCursor(id uint64) (bool, error) {
	// Get the cursor with the given ID
	cursor, err := b.getCursor(id)
	if err != nil {
		return false, err
...
func (b *SimpleBTreeEngine) ReadColumn(id uint64, column uint64) (BTreeEntry, error) {
	// Get the cursor with the given ID
	cursor, err := b.getCursor(id)
	if err != nil {
		return nil, err
	...
	cell := page.Bytes[cursor.Position:cellEnd]
	...
	for idx, typeCode := range typeCodes {
		...
		if typeCode == 0 {
			...
			entry = BTreeNumberEntry{Value: binary.BigEndian.Uint64(result)}
		} else if typeCode == 8 {
			entry = BTreeNumberEntry{Value: 0}
		} else if typeCode == 9 {
			entry = BTreeNumberEntry{Value: 1}
		} else if typeCode >= 12 && typeCode%2 == 1 {
			length := (typeCode - 12) / 2
			entry = BTreeTextEntry{Value: string(cell[pointer : pointer+length])}
			pointer += length
		} else {
	...
	return BTreeNullEntry{}, nil
}

func (b *SimpleBTreeEngine) getCursor(id uint64) (*cursor, error) {
...
func (b *SimpleBTreeEngine) getCellPointer(cellNum uint64, pageNum uint64) (*uint64, bool, error) {
	// Get the page of the table or index
	...
	err = binary.Read(bytes.NewBuffer(page.Bytes[pointer:pointer+12]), binary.BigEndian, &header)
	if err != nil {
		return nil, false, err
	}
	...
	err = binary.Read(bytes.NewBuffer(page.Bytes[cellPointerStart:cellPointerStart+2]), binary.BigEndian, &cellPointer)
	if err != nil {
		return nil, false, err
	}
